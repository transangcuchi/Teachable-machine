...
def giaodien():
    ...
    def nhandien():
        ...
        while True:
            ...
            textvat="".join(["Mệnh giá:",class_name[2:]," Đồng"])
    ...
    def anh(file_path):
        try:     
            ...
            textdtc="".join(["Độ tin cậy:",str(np.round(confidence_score * 100))[:-2],"%" ])
            vat.config(fg="black")
            dotincay.config(fg="black")                                      
            print("Class:", class_name[2:], end="")
            print("Confidence Score:", str(np.round(confidence_score * 100))[:-2], "%")
            update_frame(textvat,textdtc)
        except:
            textvat="Lỗi file ảnh"
            textdtc="Ảnh sử dụng nên là .jfif hoặc là .jpg"                          
            print("Lỗi file ảnh")
            vat.config(fg="red")
            dotincay.config(fg="red")           
            update_frame(textvat,textdtc)
            pass
    ...
    sign_image.pack(side=BOTTOM,pady=10 )
